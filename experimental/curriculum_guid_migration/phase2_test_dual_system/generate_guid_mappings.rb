...
# Generate GUID Mapping Files
# This script generates JSON mapping files that link existing unique identifiers to GUIDs
# These files will be committed to the codebase and used by the seeding process

require 'fileutils'
require 'json'

class GuidMappingGenerator
  def initialize
    @mappings_dir = '/workspace/dashboard/config/curriculum_guid_mappings'
    @output_dir = '/workspace/experimental/curriculum_guid_migration/phase2_test_dual_system'
  ...
  def generate_all_mappings
    puts "🗺️  Generating GUID Mapping Files"
    puts "=" * 60
    puts ""
  ...
  def create_mapping_summary
    ...
    summary = {
      ...
      total_files: Dir.glob(File.join(@mappings_dir, '*.json')).length,
      usage: {
    ...
    # Write summary to both locations
    File.write(File.join(@mappings_dir, 'mapping_summary.json'), JSON.pretty_generate(summary))
    File.write(File.join(@output_dir, 'mapping_summary.json'), JSON.pretty_generate(summary))
...
# Run the generator
if __FILE__ == $0
  generator = GuidMappingGenerator.new
